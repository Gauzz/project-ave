...
if (!isset($_GET["pid"]) OR empty($_GET["pid"])) {
 ...
 if (isset($_GET["pid"]) OR !empty($_GET["pid"]) ) {
    ...
    $getProject=mysqli_fetch_array($fetchProject);
     ...
     $assets = mysqli_query($conn,"SELECT * FROM assets WHERE authtoken='$getProjectToken'");
     $getAssets = mysqli_fetch_array($assets);

     $media = mysqli_query($conn,"SELECT * FROM media WHERE authtoken='$getProjectToken'");
     $getMedia = mysqli_fetch_array($media);
    
    $UserEmail=$getProject["email"];
    $projectToken=$getProject["token"];
    $projectId=$getProject["id"];

    $getProjectImg=mysqli_query($conn,"SELECT * FROM tbl_project_image WHERE token='$projectToken' ");
        $ArrImage=mysqli_fetch_array($getProjectImg);
        $ImageId= $ArrImage["id"];
  ...
  if (isset($_POST["saveproject"])) {
   ...
   if(isset($_POST["ownMag"]) AND empty($_POST["fooby"][4][0])){
    ...
    if (empty($docname)) {
        $docname=$getProject["docs"];
    }

    if (isset($_FILES["modelobj"])) {
        //Obj,fbx
    ...
    if (empty($obj_name)) {
        $obj_name=$getProject["objfile"];
        $objurlpath=$getAssets["obj"];
        $fbxurlpath=$getAssets["fbx"];
    }
    ...
    if (empty($mtl_name)) {
        $mtl_name=$getProject["mtlfile"];
        $mtlUrlPath=$getAssets["mtl"];
    }
    ...
    //video
           if (isset($_FILES["files"])) {
             ...
             $insertVideo=rand().$videoname;
             move_uploaded_file($videotempname, "uploads/video/".$insertVideo);
             $videoUrlPath = 'https://pitchar.io/uploads/video/'.$insertVideo;
           }
           if (empty($videoname)) {
             $insertVideo=$getProject["video"];
             $videoUrlPath=$getMedia["video"];
           }
...
//objThumb
             $valid_formats = array("png","jpeg","jpg");  
              ...
              if(strlen($objThumb)) {
                ...
                $getObjThumbnail=$getAssets["objthumbnail"];
           }
            ...
            //videoThumbnail
              ...
              $getVideoThumbnail=$getMedia["thumbnail"];
           }
            


    $updateProject=mysqli_query($conn,"UPDATE tbl_std_project SET project_name='$projectName',subject='$getEntr',book='$getBook',objfile='$obj_name',mtlfile='$mtl_name',video='$insertVideo',notes='$notes',docs='$docname',education='$getEdu',magaz='$getMag',grade='$grade' WHERE id='$projectId'");
    if ($updateProject) {
      // update New Assets
      $queryAssets=mysqli_query($conn,"UPDATE assets SET objthumbnail='$getObjThumbnail',obj='$objurlpath',mtl='$mtlUrlPath',fbx='$fbxurlpath',name='$projectName' WHERE authtoken='$getProjectToken'");

     // update New Media
       $queryMedia=mysqli_query($conn,"UPDATE media SET video='$videoUrlPath',thumbnail='$getVideoThumbnail',audio='$getAudioName',name='$projectName' WHERE authtoken='$getProjectToken'");
...
<html>
  ...
  $(document).ready(function() {
    ...
    $('#calendar').fullCalendar({
      ...
      events: [
        {
          title: '<?php echo $getProject["calender"] ?>',
          start: '<?php echo $getProject["start_date"] ?>',
          end: '<?php echo $getProject["end_date"] ?>'
   ...
   <body class="hold-transition skin-blue sidebar-mini">
     ...
     <form method="POST" enctype="multipart/form-data"> 
            ...
            <section class="content-header">
               <div class="row">
                  <div class="col-md-6">
                    <input   type="text" required="" value="<?php echo $getProject["project_name"] ?>"  class="form-control project-name" style="margin-top:  20px;" placeholder="Project Name" name="project_name"   >
                  </div>
                  <!-- <div class="col-md-3">
            ...
            data-i18nnofilter="No selection" data-i18nfilter="Filter" data-onchangecallback="onChangeCallback" />
                  ...
                  <div>
                    <button type="submit" name="saveproject" class="btn btn-primary pull-right publish-btn" style="margin-right: 20px;margin-top: 20px;">Save</button>
                  </div>
...
<?php 
 ...
 ?>
                  ...
                  <li><a class="get-edu" href="javascript:void(0)" data-value="<?= $getEduData["name"];?>">
                        <input <?php echo ($getProject["education"]==$getEduData["name"]) ? "checked" : " " ; ?>  type="checkbox" class="checkboxs edu-check" id="edu<?= $getEduData["id"];?>" value="<?= $getEduData["name"];?>" name="fooby[1][]" /> <label style="font-weight: normal;" for="edu<?= $getEduData["id"] ?>"><?= $getEduData["name"];?></label></a></li>
<?php } ?>                   
...
?>                   
                      <li><a class="get-entr" href="javascript:void(0)" data-value="<?= $subject_new["subject"];?>" >
                        ...
                        <input  type="checkbox" <?php echo ($getProject["subject"]==$subject_new["subject"]) ? "checked" : " " ; ?> class="checkboxs entr-check" id="set<?= $subject_new["id"];?>" value="<?= $subject_new["subject"];?>" name="fooby[2][]" /> <label style="font-weight: normal;" for="set<?= $subject_new["id"] ?>"><?= $subject_new["subject"];?></label></a></li>
...
<?php 
                  ...
                  { ?>
                                ...
                                <li><a class="get-book" data-value="<?= $book_data["book"];?>" href="javascript:void(0)">
                                 <input type="checkbox" <?php echo ($getProject["book"]==$book_data["book"]) ? "checked" : " " ; ?> class="checkboxs book-check" id="radio<?= $book_data["id"];?>" value="<?= $book_data["book"];?>" name="fooby[3][]" /> <label style="font-weight: normal;" for="radio<?= $book_data["id"] ?>"><?= $book_data["book"];?></label> </a></li>
 <?php } ?>
...
<?php 
 ...
 ?>
                  ...
                  <li>
                    ...
                    <input type="checkbox" <?php echo ($getProject["magaz"]==$getmagData["name"]) ? "checked" : " " ; ?> class="checkboxs mag-check" id="mag<?= $getmagData["id"];?>" value="<?= $getmagData["name"];?>" name="fooby[4][]" />
                              ...
                              <div class="col-lg-12">
                                 ...
                                 <div class="col-lg-10 fon">
                                  ...
                                  <select  name="grade" class="form-control" id="chooseBook">
                                         <option selected=""><?php echo $getProject["grade"] ?></option> 
                                ...
                                <option value="1 – 12 ELEMENTARY SCHOOL">1 – 12 ELEMENTARY SCHOOL</option>
                                <option value="1 – 12 MIDDLE SCHOOL">1 – 12 MIDDLE SCHOOL</option>
                                <option value="1 – 12 HIGH SCHOOL">1 – 12 HIGH SCHOOL</option>
               ...
               <div class="container">
                <div class="col-md-4">
                     <div class="panel panel-default">
                        <div class="">
                           <div class="main">
                           <?php if (empty($ArrImage["image"])) {
                               $path='dist/img/main-icon.png';
                           }
                           if (!empty($ArrImage["image"])) {
                                $path='uploads/imgs/'.$ArrImage["image"];
                           }
                            ?>                 
                          <img style="cursor: pointer;" width="100%" height="280px" src="<?php echo $path; ?>"  id="output"/>                     
                           </div>
                        </div>
                        <div class="panel-footer"id="pn"><strong data-toggle="modal" data-target="#ImageModel" class="text-info" style="cursor: pointer;">Update Images</strong>   <!-- <strong>Update Images</strong> --><br>
              <input id="fileLoader" style="display: none;" multiple  type="file" accept="image/*" onchange="loadFile(event)" name="images[]" >
                    ...
                    <div class="col-md-4" id="doc">
                     <div  class="panel panel-default">
                      ...
                      <?php if (!empty($getProject["docs"])): ?>
                        <?php 
                        $ext=substr($getProject["docs"], -3);
                        if ($ext=='doc' OR $ext=='docx' OR $ext=='DOC' OR $ext=='DOCX'): ?>
                          <?php $css='display:none;'; ?>
                          <?php $path=''; ?>
                           <img id="upload-button" class="img-responsive" src="https://cdn0.iconfinder.com/data/icons/iconico-3/1024/48.png">
                          <?php else: ?>
                          <?php $css='display:block;'; ?>
                           <?php $path='uploads/docs/'.$getProject["docs"]; ?>
                          
                        <?php endif ?>


                         <iframe style="<?php echo $css; ?>" height="200" id="upload-button" src="<?php echo $path; ?>" class="img-responsive"></iframe>
                      <?php endif ?>
                     <!--  <img id="upload-button" class="img-responsive" src="dist/img/main-icon.png"> -->
                      
                      <!--   <button type="button" id="upload-button">Select PDF</button>  -->
                        <input accept=".pdf,.doc" style="display: none;" name="myPDF" type="file" id="file-to-upload" />
                        ...
                        <div class="panel-footer" id="pn" ><strong class="update-doc text-info" style="cursor: pointer;">Update Document</strong><br>
                     ...
                     <div class="panel panel-default">
                        <div class="">
                           <div class="main">
                            ...
                            <?php endif ?>
                            <?php if (!empty($getProject["objfile"])): ?>
                              <iframe style="width: 100%;height: 275px;" src="obj-viewer.php?token=<?php echo $projectToken; ?>"></iframe>
                            <?php endif ?>
                                
                           </div>
                        </div>
                        <div class="panel-footer"id="pn"><strong class="text-info" onclick="openfileobj();" style="cursor: pointer;" class="updateModel">Update Model</strong> <small>(Supports OBJ,FBX)</small><br>
                          ...
                          <div class="container">
                            <div class="col-md-4">
                              <div class="panel panel-default">
                                <div class="">
                                  <div class="main">
                                    <img onclick="objthumbnail();" style="cursor: pointer;" width="100%" height="280px" src="<?php echo (!empty($getAssets["objthumbnail"])) ? $getAssets["objthumbnail"] : 'dist/img/main-icon.png' ;?>"  id="blah"/>
                                  </div>
                                </div>
                              ...
                              <div class="panel panel-default">
                                <div class="main">
                                  ...
                                  <audio controls style="position: absolute;left: 16px;bottom: 88px; width: 91%;">
                                    <source id="audiosrc" src="<?php echo $getMedia["audio"]?>" type="audio/mp3">
                                  </audio>
                            ...
                            <div class="col-md-4">
                              <div class="panel panel-default">
                                <div class="">
                                  <div class="main">
                                    <img onclick="videoThumbFile();" id="videothumb" src="<?php echo (!empty($getMedia["thumbnail"])) ? $getMedia["thumbnail"]:'dist/img/main-icon.png'?>" width="100%" height="280px" style="cursor: pointer;">
                                  </div>
                                </div>
                  ...
                  <div class="col-md-12">
                     ...
                     <div class="panel panel-default" id="m">
                        <div class="panel-body" style="padding: 0;cursor: pointer;" id="videoFileUpload1">
                          <?php if(empty($getProject["video"])): ?>
                          <img src="https://arvrintedu.com/dist/img/main-icon.png" id="videoPlaceholder" height="600px" width="100%">
                        <?php endif; ?>
                        <?php if (!empty($getProject["video"])): ?>
                  ...
                  <div class="col-md-12">
                     <div class="panel panel-default" > 
                        <div class="panel-body">   
                            ...
                            <textarea   class="form-control" name="notes" placeholder="Writer Your Notes Here..."><?php 
                              if(!empty($getProject["notes"])){
                                echo "";
                              }
                              if(!empty($getProject["notes"])){
                                echo $getProject["notes"];
                              }
                             ?></textarea>
                        </div>
                    </div>
...
<div class="modal fade" id="ImageModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
         <?php   
          $getProjectImg2=mysqli_query($conn,"SELECT * FROM tbl_project_image WHERE token='$projectToken' ");
           while($fetchImages=mysqli_fetch_array($getProjectImg2)){ 
                
        ?>

        <form method="POST" class="uploadForm" data-name="form<?php echo $fetchImages["cnt"]?>" enctype="multipart/form-data">
            <div id="uploadFormLayer">
 
              <img id="preview-img<?php echo $fetchImages["cnt"]?>" style="margin-bottom: 20px" height="100%" width="100%" src="uploads/imgs/<?php echo $fetchImages["image"]; ?>">
 
                     <input type="hidden" name="set" value="<?php echo $fetchImages["id"]?>">
                     <!-- <input  type="file" id="uploadimage1" onchange="ImageFile(event)" name="userImage" style="display: none;"> -->
                     <input accept="image/*" style="display: none;" type="file" id="uploadimage<?php echo $fetchImages["cnt"]?>" onchange="document.getElementById('preview-img<?php echo $fetchImages["cnt"]?>').src = window.URL.createObjectURL(this.files[0])" name="userImage" >
                     <button type="button" data-btns="<?php echo $fetchImages["cnt"]?>" onclick="updateImg(<?php echo $fetchImages["cnt"]?>);" style=" " class="btn btn-info  image<?= $fetchImages["id"]?>"> Upload New Image</button>
                     <!-- <button type="button" style="margin-bottom: 20px" class="btn btn-outline-danger"> Delete Image</button> -->
                     <button type="submit" style=" "  class="btn btn-primary">Save changes</button>

                      <div class="alert alert-success form<?php echo $fetchImages["cnt"]?>">dfsdf</div>
                      <input type="hidden" value="<?php echo $getProjectToken ;?>" name="token">
       ...
       <hr>
<?php } ?>
        <div id="new-ImagePreview" style="display: none;">
        <img id="imagePreview" style="margin-bottom: 20px;" height="100%" width="100%" src="uploads/imgs">
        <input onchange="document.getElementById('imagePreview').src = window.URL.createObjectURL(this.files[0]);$('#new-ImagePreview').show();" type="file" style="display: none;" name="newImage" id="newImage">
        <button class="btn btn-primary"><i class="fa fa-floppy-o" aria-hidden="true"></i></button>
        <button class="btn btn-warning" id="remove-Image"><i class="fa  fa-trash" aria-hidden="true"></i></button>
        </div>
      ...
      <div class="modal-footer">
        <button type="button"  class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="submit" name="saveimage" class="btn btn-primary" onclick="$('#newImage').trigger('click');">Upload New</button>
      </div>
                                 ...
                                 function openfileDialog() {
                                    ...
                                    $(".uploadForm").on('submit',(function(e) {
                                        var formName=$(this).data("name");
                                            e.preventDefault();
                                            $("."+formName).html('<strong class="text-info">Uploading..Please Wait!</strong>');
                                            $.ajax({
                                                url: "assets/_php/updateImage.php",
                                                type: "POST",
