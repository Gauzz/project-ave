    ...
    if ($getTotalNo>0) {
    	...
    	if ($acc_status=='1') {
			...
			//	$response['data']=$verifypass;
				...
				$response['data']="".$verifypass['id'].' ,'.$verifypass['firstname'].' ,'. $verifypass['lastname'].' , '.$verifypass['country'].','. $verifypass['email'] .', '. $verifypass['occupation'].','. $verifypass['authtoken'].','.$imageUrl;
    		 }
