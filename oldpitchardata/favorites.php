                  ...
                  <div class="col-md-6">
                     <ol class="breadcrumb">
                        <div class="input-group">
                           ...
                           <input required="" type="text" name="query" class="form-control" placeholder="Search Favorites...">
...
if (isset($_GET["query"])) {
   $query=$_GET["query"];
   $query=mysqli_query($conn,"SELECT * FROM favourite WHERE email='$getEmail' AND fav='1' AND uid LIKE '%$query%'  ");
...
while($getFavs=mysqli_fetch_array($query)){
   $favToken=$getFavs["token"];
   $getFavPro=mysqli_query($conn,"SELECT * FROM tbl_std_project WHERE token='$favToken' ");
   $data=mysqli_fetch_array($getFavPro);
?>
...
<?php } 
if ($countResults=="0") {
  echo  "<p class='text-center h5'>Nothing To Fetch..</p>";
}
