...
if (!empty($_REQUEST["create"]) AND isset($_REQUEST["create"])) {
	...
	if (isset($_FILES["obj"]["name"]) AND !empty($_FILES["obj"]["name"])) {
		...
		$objurlpath = ($shortObFbx == ".obj" OR $shortObFbx == ".OBJ") ? 'http://localhost/user/uploads/obj/' . $modelName : '';
		$fbxurlpath = ($shortObFbx == ".fbx" OR $shortObFbx == ".FBX") ? 'http://localhost/user/uploads/obj/' . $modelName : '';
	...
	if (isset($_FILES["mtl"]["name"]) AND $_FILES["mtl"]["size"] > 0) {
		...
		$mtlUrlPath = 'http://localhost/user/uploads/mtl/' . $mtlName;
	...
	if (isset($_FILES["gltf"]["name"]) AND $_FILES["gltf"]["size"] > 0) {
		...
		$gltfUrlPath = 'http://localhost/user/uploads/obj/' . $gltfName;
		...
		if (strlen($image)) {
			...
			if (in_array($ext, $valid_formats)) {
				if ($sizeimage < (10240 * 10240)) {
					...
					move_uploaded_file($tmp, "../uploads/img/" . $imageName);
					$imageUrlPath = 'http://localhost/user/uploads/img/' . $imageName;
				}
			}
		} else {
			$imageUrlPath = '';
		...
		if (strlen($objThumb)) {
			...
			if (in_array($ext, $valid_formats)) {
				if ($sizeobj < (10240 * 10240)) {
					...
					$getObjThumbnail = 'http://localhost/user/uploads/objThumbnail/' . $objThumbnailName;
				}
			}
		} else {
			$getObjThumbnail = '';
